					...
					<tbody>
						...
						if ( !empty( $results ) && !is_wp_error( $results ) ):
							...
							<?php foreach ( $results as $group ) : ?>
								...
								<tr>
									<td>
										<input type="hidden" name="pricing_rules[<?php echo $escaped_name; ?>][conditions_type]" value="all" />
										<input type="hidden" name="pricing_rules[<?php echo $escaped_name; ?>][conditions][<?php echo $condition_index; ?>][type]" value="apply_to" />
										<input type="hidden" name="pricing_rules[<?php echo $escaped_name; ?>][conditions][<?php echo $condition_index; ?>][args][applies_to]" value="groups" />
										<input type="hidden" name="pricing_rules[<?php echo $escaped_name; ?>][collector][type]" value="always" />
										<input class="checkbox" <?php echo $checked; ?> type="checkbox" id="group_<?php echo $group_id; ?>" name="pricing_rules[<?php echo $escaped_name; ?>][conditions][<?php echo $condition_index; ?>][args][groups][]" value="<?php echo $escaped_group_id; ?>" />
									</td>
									<td>
									...
									<td style="display:none;">
										...
										<input <?php checked( 'yes', $rule['free_shipping'] ); ?> type="checkbox" name="pricing_rules[<?php echo $escaped_name; ?>][rules][<?php echo $index; ?>][free_shipping]" value="yes" />

									</td>
									<td>
										<select id="pricing_rule_type_value_<?php echo $escaped_name . '_' . $index; ?>" name="pricing_rules[<?php echo $escaped_name; ?>][rules][<?php echo $index; ?>][type]">
									...
									<td>
										<input type="text" name="pricing_rules[<?php echo $escaped_name; ?>][rules][<?php echo $index; ?>][amount]" value="<?php echo esc_attr( $rule['amount'] ); ?>" />
									</td>
								</tr>
							<?php endforeach; ?>
						<?php endif; ?>
