		...
		foreach ( $terms as $item_id => $item ) :
			...
			<tr>
				<td>
					...
					<input type="hidden" name="pricing_rules[<?php echo $escaped_name; ?>][conditions][0][type]" value="apply_to" />
					<input type="hidden" name="pricing_rules[<?php echo $escaped_name; ?>][conditions][0][args][applies_to]" value="everyone" />
					<input type="hidden" name="pricing_rules[<?php echo $escaped_name; ?>][collector][type]" value="cats" />
					<input class="checkbox" <?php echo $checked; ?> type="checkbox" id="item_<?php echo $item->term_id; ?>" name="pricing_rules[<?php echo $escaped_name; ?>][collector][args][cats][]" value="<?php echo $item->term_id; ?>" />
				</td>
				<td>
					<?php if ( $item->parent ) : ?>
		...
		?>
			<div id="woocommerce-pricing-collector-<?php echo $name; ?>" class="section" style="" >
				...
				<select title="Choose how to calculate the quantity.  This tallied amount is used in determining the min and max quantities used below in the Quantity Pricing section." class="pricing_rule_when" id="pricing_rule_when_<?php echo $name; ?>" name="pricing_rules[<?php echo $name; ?>][collector][type]">
					...
					<option title="Calculate quantity based on total sum of the categories in the cart, per product" <?php selected( 'cat_per_product', $collector['type'] ); ?> value="cat_per_product"><?php _e( 'Sum of Category Per Product', 'woocommerce-dynamic-pricing' ); ?></option>
				</select>
			</div>
