...
class WC_Multistore_Abstract_Product_Child {
	...
	public function __construct( $product ) {
		...
		if ( is_a( $product, 'WC_Product' ) ) {
			...
			$slave_id = wc_multistore_product_get_slave_product_id( $product['ID'], $product['sku'], $product['language'] );
			if ( $slave_id ) {
		...
		$this->language = apply_filters( 'wc_multistore_child_product_language', false, $this->wc_product );
	...
	public function create() {
		...
		if ( $this->site_settings['child_inherit_changes_fields_control__status'] == 'yes' ) {
			...
			$this->wc_product->set_status( $this->site_settings['child_inherit_changes_fields_control__default_status'] );
		}
		...
		if ( wc_multistore_has_gtin_version() && $this->site_settings['child_inherit_changes_fields_control__gtin'] == 'yes'){
			if( ! empty( $this->data['gtin'] ) && ! wc_product_has_global_unique_id( $this->wc_product->get_id() , $this->data['gtin'] ) ){
				$this->duplicate_gtin = true;
			}else{
				$this->wc_product->set_global_unique_id( $this->data['gtin'] );
			}
		}
		...
		if( $this->site_settings['child_inherit_changes_fields_control__default_variations'] == 'yes' ){
			if( isset( $this->data['default_attributes'] ) ){
				$this->wc_product->set_default_attributes( $this->get_default_attributes() );
			}else{
	...
	public function update() {
		...
		if ( $this->has_stock_sync_enabled()  ) {
			...
			if ( $this->site_settings['child_inherit_changes_fields_control_update__allow_backorders'] == 'yes' ) {
				$this->wc_product->set_backorders( $this->data['backorders'] );
			}
		}
		...
		if ( $this->has_publish_changes_enabled() ) {
			...
			if ( $this->site_settings['child_inherit_changes_fields_control_update__title'] == 'yes' ) {
				$this->wc_product->set_name( $this->data['post_title'] );
			}

			if ( $this->site_settings['child_inherit_changes_fields_control_update__slug'] == 'yes' ) {
				$this->wc_product->set_slug( $this->data['slug'] );
			}
			...
			if ( $this->site_settings['child_inherit_changes_fields_control_update__status'] == 'yes' ) {
				$this->wc_product->set_status( $this->data['post_status'] );
			}

			if ( $this->site_settings['child_inherit_changes_fields_control_update__featured'] == 'yes' ) {
				$this->wc_product->set_featured( $this->data['featured'] );
			}

			if ( $this->site_settings['child_inherit_changes_fields_control_update__catalogue_visibility'] == 'yes' ) {
				$this->wc_product->set_catalog_visibility( $this->data['catalog_visibility'] );
			}

			if ( $this->site_settings['child_inherit_changes_fields_control_update__description'] == 'yes' ) {
				$this->wc_product->set_description( $this->data['description'] );
			}

			if ( $this->site_settings['child_inherit_changes_fields_control_update__short_description'] == 'yes' ) {
				$this->wc_product->set_short_description( $this->data['post_excerpt'] );
			}

			if ( $this->site_settings['child_inherit_changes_fields_control_update__sku'] == 'yes' ) {
				if( apply_filters( 'wc_multistore_set_sku', true, $this->data, $this->wc_product ) ){
					if( empty($this->data['sku']) && WOO_MULTISTORE()->settings['sync-by-sku'] == 'yes' ){
						$this->duplicate_sku = true;
					}
			...
			if ( wc_multistore_has_gtin_version() && $this->site_settings['child_inherit_changes_fields_control_update__gtin'] == 'yes'){
				if( ! empty( $this->data['gtin'] ) && ! wc_product_has_global_unique_id( $this->wc_product->get_id() , $this->data['gtin'] ) ){
					$this->duplicate_gtin = true;
				}else{
					$this->wc_product->set_global_unique_id( $this->data['gtin'] );
			...
			if ( $this->site_settings['child_inherit_changes_fields_control_update__price'] == 'yes' ) {
				$this->wc_product->set_price( $this->data['price'] );
				$this->wc_product->set_regular_price( $this->data['regular_price'] );
			}

			if ( $this->site_settings['child_inherit_changes_fields_control_update__sale_price'] == 'yes' ) {
				$this->wc_product->set_sale_price( $this->data['sale_price'] );
			...
			if ( $this->site_settings['child_inherit_changes_fields_control_update__upsell'] == 'yes' ) {
				$this->wc_product->set_upsell_ids( $this->get_upsell_ids() );
			}

			if ( $this->site_settings['child_inherit_changes_fields_control_update__cross_sells'] == 'yes' ) {
				$this->wc_product->set_cross_sell_ids( $this->get_cross_sell_ids() );
			}

			$this->wc_product->set_parent_id( $this->get_parent_id() );

			if ( $this->site_settings['child_inherit_changes_fields_control_update__purchase_note'] == 'yes' ) {
				$this->wc_product->set_purchase_note( $this->data['purchase_note'] );
			}

			if ( $this->site_settings['child_inherit_changes_fields_control_update__attributes'] == 'yes' ) {
				$this->wc_product->set_attributes( $this->get_attributes() );
			}

			if( $this->site_settings['child_inherit_changes_fields_control_update__default_variations'] == 'yes' ){
				if( isset( $this->data['default_attributes'] ) ){
					$this->wc_product->set_default_attributes( $this->get_default_attributes() );
				}else{
					$this->wc_product->set_default_attributes( array() );
			...
			if ( $this->site_settings['child_inherit_changes_fields_control_update__menu_order'] == 'yes' ) {
				$this->wc_product->set_menu_order( $this->data['menu_order'] );
			}
			...
			if ( $this->site_settings['child_inherit_changes_fields_control_update__product_cat'] == 'yes' ) {
				$this->wc_product->set_category_ids( $this->get_category_ids() );
			}

			if ( $this->site_settings['child_inherit_changes_fields_control_update__product_tag'] == 'yes' ) {
				$this->wc_product->set_tag_ids( $this->get_tag_ids() );
			}

			$this->wc_product->set_downloads( $this->get_downloads() );

			if( $this->site_settings['child_inherit_changes_fields_control_update__shipping_class'] == 'yes' ){
				$this->wc_product->set_shipping_class_id($this->get_shipping_class_id());
			}

			if ( $this->site_settings['child_inherit_changes_fields_control_update__product_image'] == 'yes' ) {
				$this->wc_product->set_image_id( $this->get_image_id() );
			}

			if ( $this->site_settings['child_inherit_changes_fields_control_update__product_gallery'] == 'yes' ) {
				$this->wc_product->set_gallery_image_ids( $this->get_gallery_image_ids() );
			}
			...
			if( $this->site_settings['child_inherit_changes_fields_control_update__reviews'] == 'yes' ){
				$this->wc_product->set_reviews_allowed( $this->data['reviews_allowed']);
	...
	public function save() {
		...
		if( $this->duplicate_gtin ){
			...
			return array(
				'status' => 'failed',
				'message' => 'Failed to sync #'. $this->data['ID'].' to '. $site->get_url() .  ' - Duplicate GTIN, UPC, EAN, or ISBN found',
				'code' => '500'
			);
		}
	...
	public function get_upsell_ids() {
		...
		if ( ! empty( $upsells ) ) {
			foreach ( $upsells as $upsell_id => $upsell_sku ) {
				$child_upsell_id = wc_multistore_product_get_slave_product_id( $upsell_id, $upsell_sku, $this->data['language'] );
	...
	public function get_cross_sell_ids() {
		...
		if ( ! empty( $cross_sells ) ) {
			foreach ( $cross_sells as $cross_sell_id => $cross_sell_sku ) {
				$child_cross_sell_id = wc_multistore_product_get_slave_product_id( $cross_sell_id, $cross_sell_sku, $this->data['language'] );
...
//		}
	...
	public function get_parent_id() {
		...
		return wc_multistore_product_get_slave_product_id( $this->data['post_parent'], $this->data['sku'], $this->data['language'] );
	}
	...
	public function get_category_ids() {
		...
		foreach ( $product_category_ids as $product_category_id => $categories ) {
			foreach ( $categories as $category ) {
				...
				$wc_multistore_child_category->save();
			}
	...
	public function get_tag_ids() {
		...
		foreach ( $product_tag_ids as $tag ) {
			...
			$wc_multistore_product_tag_child->save();
	...
	public function get_shipping_class_id(){
		...
		$wc_multistore_product_shipping_class_child->save();
	...
	public function get_default_attributes() {
		$attributes = $this->data['default_attributes'];
		if( empty($attributes) ){
			return $attributes;
		}
		...
		foreach ($attributes as $term_taxonomy_id => $id){
			if (str_contains($term_taxonomy_id,'pa_')){
				$attributes_data[str_replace('pa_','',$term_taxonomy_id)] = $id;
			}else{
				$child_attibute_id = wc_multistore_get_child_term_id($id);
				$child_attribute_id = wc_multistore_get_child_product_attribute_by_id($term_taxonomy_id);
				$taxonomy = wc_attribute_taxonomy_name_by_id( intval( $child_attribute_id ) );

				if( $child_attribute_id ){
					$child_term = get_term( $child_attibute_id, $taxonomy );
					if( $child_term && ! is_wp_error($child_term) ){
						$attributes_data[$taxonomy] = $child_term->slug;
					}
				}
			}
		}
	...
	public function sync_stock_to_master(){
		...
		if( is_multisite() ){
			...
			switch_to_blog(get_site_option('wc_multistore_master_store'));
			$master_product_id = wc_multistore_product_get_master_product_id( $this->wc_product->get_meta('_woonet_network_is_child_product_id'), $this->wc_product->get_sku(), $this->language );
	...
	public function delete_sync_data_from_master(){
		if( is_multisite() ){
			...
			$result = $wc_multistore_child_product_api->send_delete_sync_data_from_master(  $this->wc_product->get_meta('_woonet_network_is_child_product_id'), $this->wc_product->get_meta('_woonet_network_is_child_product_sku'), $this->language );
		}
