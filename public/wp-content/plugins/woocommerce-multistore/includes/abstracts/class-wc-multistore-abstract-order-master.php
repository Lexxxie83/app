...
//		$data['completed_at'] = $this->wc_order->get_order_number();
		...
		return apply_filters( 'wc_multistore_original_order_data', $data, $this->wc_order );
	}
	...
	public function product_to_json( $product_id ) {
		...
		$product['meta'] = apply_filters('wc_multistore_clone_product_meta', $product['meta'], $wc_product );
		...
		if ( $product_attributes = $wc_product->get_attributes() ) {
			...
			foreach ( $product_attributes as $pa ) {
				...
				$attr = array(
					...
					'master_id' => wc_multistore_get_master_attribute($pa->get_id())
				);
		...
		if ( $wc_product->get_type() == 'variable' ) {
			...
			foreach ( $variations as $variation ) {
				...
				$variation_meta = apply_filters( 'wc_multistore_clone_variation_meta', $variation_meta, $wc_variation );
	...
	private function update_order_meta( $order_meta ) {
		...
		if ( ! empty( $order_meta_data ) && is_array( $order_meta_data ) ) {
			foreach ( $order_meta_data as $meta_data ) {
				if( $meta_data['key'] == '_order_number' ){
					continue;
				}
				if( apply_filters('wc_multistore_import_order_meta_key', true, $meta_data['key']) ){
					$this->wc_order->update_meta_data( $meta_data['key'], wp_unslash($meta_data['value']) );
				}
			}
		}
	}
	...
	public function update($data){
		...
		do_action('wc_multistore_before_master_order_save', $this->wc_order, $data );
