...
//		'_product_version',
	...
	public function __construct( $wc_product, $depth = false ) {
		...
		$this->load_settings();
		$this->language = apply_filters( 'wc_multistore_master_product_language', false, $this->wc_product );
	...
	public function get_data() {
		if ( class_exists( 'SitePress' ) ) {
			if (!empty($this->language) && !empty($this->language['language_code'])){
				do_action( 'wpml_switch_language', $this->language['language_code'] );
			}
		}
		...
		$data['default_attributes'] = $this->get_default_attributes();
		$data['menu_order']         = $this->wc_product->get_menu_order('edit');
		$data['post_password']      = $this->wc_product->get_post_password('edit');
		...
		if ( wc_multistore_has_gtin_version() ){
			$data['gtin']           = $this->wc_product->get_global_unique_id('edit');
		}
	...
	public function get_default_attributes() {
		$attributes = $this->wc_product->get_default_attributes('edit');
...
//			}
	...
	public function sync_stock_to($site_id) {
		if( $this->should_sync_stock_to( $site_id ) ) {
			if( is_multisite() ){
				switch_to_blog( $site_id );
                $slave_id = wc_multistore_product_get_slave_product_id( $this->data['ID'], $this->data['sku'], $this->data['language'] );
                if (!$slave_id){
                    restore_current_blog();
                    return false;
                }
				$classname                = wc_multistore_get_product_class_name( 'child', $this->wc_product->get_type() );
				$wc_multistore_child_product = new $classname( $this->data );
				$wc_multistore_child_product->update_stock($this->wc_product->get_stock_quantity('edit'));
				restore_current_blog();
			}else{
	...
	public function set_ajax_transient($transient_name) {
		...
		$data[$this->wc_product->get_id()]['language'] = $this->data['language'];
	...
	public function set_scheduler($hook) {
		$scheduled_actions = as_get_scheduled_actions(
			array(
				'hook'   => $hook,
				'args'   => array( 'id' => $this->wc_product->get_id(), 'sku' => $this->wc_product->get_sku('edit'), $this->data['language'] ),
		...
		as_enqueue_async_action($hook, array( $this->wc_product->get_id(), $this->wc_product->get_sku('edit'), $this->data['language'] ),'WooMultistore Product Sync' );
	}
}