...
//		$this->wc_order->update_status( $this->data['status'], 'Imported order via API from child site.', TRUE );
		...
		if( apply_filters('wc_multistore_import_order_customer', true ) ) {
			if(empty($this->data['customer_data'])){
				...
				if( apply_filters('wc_multistore_import_order_customer_real_data', false ) ){
					$customer_id = $this->add_update_customer_real_data();
				}else{
	...
	public function save(){
		...
		do_action('wc_multistore_after_import_order_save', $this->wc_order, $this->data );
		...
		if ( isset($this->data['coupon_items']) ) {
			foreach ( $this->data['coupon_items'] as $coupon_item ) {
				...
				if ( $coupon_item_id ) {
					...
					wc_add_order_item_meta( $coupon_item_id, 'coupon_info', $coupon_info );
				}
			}
		}
	}
	...
	private function add_order_items( $new_tax_items_rate_id ) {
		...
		foreach ( $this->data['line_items'] as $key => $item ) {
            ...
            if ( ! empty( $item['unsynced_product'] ) && WOO_MULTISTORE()->settings['clone-products-during-import-order'] == 'yes' ) {
                ...
                do_action('wc_multistore_process_cloned_product', $linked_product_id, $item, $this->data['site_id']);

                // Clone attributes to main site if the product is a clone
                if ($cloned) {
	...
	public function pre_insert_user_data( $data, $update, $user_id, $userdata ){
		if( $update && ! empty( $userdata ) && ! empty( $userdata['user_login'] )){
	...
	public function add_update_customer() {
		...
		$site_id = ( is_multisite() ) ? $this->data['site_id'] : substr($this->data['site_id'], 0, 7);
		$new_user_username  = 'childsite_' . $site_id . '_' . $this->data['customer_data']['user_nicename'];
		$new_user_password  = 'childsite_' . $site_id . '_' . $this->data['customer_data']['user_nicename'];
		$new_user_email     = 'childsite_' . $site_id . '+' . $this->data['customer_data']['user_email'];

		if ( ! empty( $mapped_customer->user_id ) ) {
			$user_id = $mapped_customer->user_id;
			$user_obj = get_userdata( $user_id );
			$user = $user_obj->to_array();
			$user['user_login'] = $new_user_username;
			$user['user_pass'] = $new_user_password;
			$user['user_email'] = $new_user_email;
			add_filter( 'wp_pre_insert_user_data', array( $this,'pre_insert_user_data' ), 10, 4 );
			wp_update_user( $user );
			remove_filter( 'wp_pre_insert_user_data', array( $this,'pre_insert_user_data' ) );
		} else {
			$user_id = wp_create_user( $new_user_username, $new_user_password, $new_user_email );
	...
	public function add_update_customer_real_data() {
		$customer_email = $this->data['customer_data']['user_email'];
		$customer_username = $this->data['customer_data']['user_login'];
		$customer_password = $this->data['customer_data']['user_pass'];
		...
		if ( ! empty( $user ) ) {
			...
			$user_id = wp_create_user( $customer_username, $customer_password, $customer_email );
		}
	...
	private function remove_order_meta() {
		...
		foreach ( $child_order_meta['meta_data'] as $child_order_meta ) {
			if( $child_order_meta->key == '_order_number' ){
				continue;
			}
	...
	private function add_order_meta() {
		...
		$order_number = $this->wc_order->get_meta('_order_number');
		foreach ( $this->data['meta_data'] as $order_meta_key => $order_meta_value ) {
			if ( ! empty( $order_number ) && $order_meta_key == '_order_number') {
				continue;
			}

			if( apply_filters('wc_multistore_import_order_meta_key', true, $order_meta_key) ){
				$this->wc_order->update_meta_data( $order_meta_key, wp_unslash($order_meta_value) );
			}
		}
	}
	...
	public function clone_product( $original_product ){
		...
		if (! empty(['unsynced_product']['meta']['_regular_price'])){
		...
		do_action('wc_multistore_before_clone_product_save',$cloned_product, $original_product);
		...
		if( $product_type == 'variable' ){
			if( $original_product['unsynced_product']['product_variations'] ){
				...
				foreach ( $original_product['unsynced_product']['product_variations'] as $original_variation ){
					...
					do_action('wc_multistore_before_clone_variation_save',$cloned_product, $original_variation);
	...
	public function get_data(){
		...
		return apply_filters( 'wc_multistore_imported_order_data', $order_data, $this->wc_order );
	}
