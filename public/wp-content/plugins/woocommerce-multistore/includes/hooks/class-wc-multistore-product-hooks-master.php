...
class WC_Multistore_Product_Hooks_Master{
	...
	public function hooks(){
		...
		add_action( 'wc_multistore_scheduled_delete_products', array( $this, 'run_scheduled_products_delete' ), 10, 3 );
	...
	public function run_scheduled_products_delete( $post_id, $sku, $language ) {
		$sites = WOO_MULTISTORE()->sites;
		unset($sites[get_current_blog_id()]);
		foreach ( $sites as $site ){
			if( is_multisite() ){
				switch_to_blog($site->get_id());
				$child_product_id = wc_multistore_product_get_slave_product_id($post_id, $sku, $language);
				if($child_product_id){
				...
				$response = $wc_multistore_product_api_master->send_delete_product_data_to_child($args,$site->get_id());
