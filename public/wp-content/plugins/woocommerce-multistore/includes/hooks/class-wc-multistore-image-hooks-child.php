...
class WC_Multistore_Image_Hooks_Child{
	...
	public function hooks(){
		...
		add_filter('woocommerce_rest_prepare_product_variation_object', array( $this, 'filter_variation_response' ), 10, 3);
	}
	...
	public function image_downsize( $attachment_id, $image_data, $size ){
		...
		if ( $intermediate ) {
			...
			$thumb_file = isset( $image_data['meta_data']['thumb'] ) ? $image_data['meta_data']['thumb'] : false;
			$info       = null;
...
//			}
	...
	public function load_variations() {
		...
		if ( $variations ) {
			...
			foreach ( $variations as $variation_object ) {
				...
				$compare        = '9.3.0';
				if (version_compare( $wc_version, $compare, '<' )){
					include WOO_MSTORE_PATH. '/includes/admin/views/html-variation-admin.php';
                } else{
					include WOO_MSTORE_PATH. '/includes/admin/views/html-variation-admin-9.3.0.php';
                }
				$loop++;
			}
		}
	...
	function filter_product_response($response, $product, $request){
		$response->data['images'] = $this->get_images($product);

		return $response;
	}

	function filter_variation_response($response, $product, $request){
		$response->data['image'] = current($this->get_images($product));
		return $response;
	}
	...
	function get_images( $product ) {
		...
		foreach ( $attachment_ids as $position => $attachment_id ) {
			...
			$idPrefix = 1000000;
			if ( $this->id_prefix_included_in_attachment_id( $attachment_id, $idPrefix ) ) {
				$globalAttachmentId = $this->strip_site_id_prefix_from_attachment_id( $idPrefix, $attachment_id );
				if( is_multisite() ){
					switch_to_blog($this->master_site);
					$props= wc_get_product_attachment_props( $globalAttachmentId );
					$attachment_alt = $props['alt'];
					$attachment_title = $props['title'];
					restore_current_blog();
				}else{
					$global_image_data = wc_multistore_get_global_image_metadata($globalAttachmentId);
					if( is_array( $global_image_data ) ){
						$image_data = $global_image_data;
						$upload_dir = $global_image_data['uploads_dir'];

						$props = $this->get_product_attachment_props( $image_data, $upload_dir, $globalAttachmentId );
						$attachment_alt = $props['alt'];
						$attachment_title = $props['title'];
					}
				}
				...
				$attachment_title = get_post_meta( $attachment_id, '_wp_attachment_image_alt', true );
			}
