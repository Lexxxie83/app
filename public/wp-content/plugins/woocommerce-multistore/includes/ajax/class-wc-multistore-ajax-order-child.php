...
class WC_Multistore_Ajax_Order_Child {
	function __construct() {
		...
		add_action( 'wp_ajax_wc_multistore_delete_unfinished_order', array( $this, 'wc_multistore_delete_unfinished_order' ) );
		add_action( 'wp_ajax_nopriv_wc_multistore_delete_unfinished_order', array( $this, 'wc_multistore_delete_unfinished_order' ) );
	}
...
//					$order = wc_get_order( $order['ID'] );
	 ...
	 *
	 */
	public function wc_multistore_delete_unfinished_order(){
		if( empty($_REQUEST['key']) || $_REQUEST['key'] != WOO_MULTISTORE()->site->get_id() ){
			return array(
				'status' => 'failed',
				'message' => 'You do not have sufficient permissions'
		...
		$original_order_id = $_REQUEST['data']['original_order_id'];
		$unfinished_orders = get_option('wc_multistore_unfinished_orders');
		if( ! empty( $unfinished_orders ) ){
			unset($unfinished_orders[$original_order_id]);
			update_option('wc_multistore_unfinished_orders', $unfinished_orders );
		}

		$id = as_unschedule_action('wc_multistore_scheduled_order', array( 'order_id' => (int) $original_order_id )  );
