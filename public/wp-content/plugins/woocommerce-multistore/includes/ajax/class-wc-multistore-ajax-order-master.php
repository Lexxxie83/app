...
class WC_Multistore_Ajax_Order_Master {
	...
	public function wc_multistore_import_order(){
		...
		// order came from child store checkout
		if( ! isset( $_REQUEST['data'] ) ){
			$args = array(
				'original_order_id' => 	$_REQUEST['id']
			);
			$wc_multistore_order_api_master = new WC_Multistore_Order_Api_Master();
			$wc_multistore_order_api_master->delete_unfinished_order_from_child_store( $args, $_REQUEST['key'] );
		}
