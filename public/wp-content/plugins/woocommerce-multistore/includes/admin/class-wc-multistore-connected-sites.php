...
class WC_Multistore_Connected_Sites {
	...
	public function add_bar_menu_page( $admin_bar ) {
		...
		$url = network_admin_url( 'admin.php?page=woonet-woocommerce-sites' );
		if( ! is_multisite() ){
			$url = network_admin_url( 'admin.php?page=woonet-connected-sites' );
		}

		$admin_bar->add_menu( array(
			'id'    => 'woonet-network-sites',
			'parent' => 'woonet-woocommerce',
			'group'  => null,
			'title' => __( 'Sites', 'woonet' ),
			'href'  => $url,
			'meta' => [
				'title' => __( 'Sites', 'woonet' ),
			]
