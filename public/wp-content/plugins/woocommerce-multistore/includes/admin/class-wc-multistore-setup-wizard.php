...
class WC_Multistore_Setup_Wizard {
	...
	public function __construct() {
		...
		if( is_multisite() ){
			...
			$this->wc_multistore_migrate_settings_multisite_5_3_0();
		}else{
			$this->wc_multistore_migrate_settings_single_master_4_8_0();
			$this->wc_multistore_migrate_settings_single_child_4_8_0();
			$this->wc_multistore_migrate_settings_single_master_5_3_0();
			$this->wc_multistore_migrate_settings_single_child_5_3_0();
		}
	...
	public function get_db_version(){
		...
		if( ! empty( $settings ) && !empty( $settings['db_version'] )  ){
	...
	public function add_bar_menu_page($admin_bar) {
		$admin_bar->add_menu( array(
			'id'    => 'woonet-setup-wizard',
			'parent' => 'woonet-woocommerce',
			'group'  => null,
			'title' => __( 'Setup Wizard', 'woonet' ), //you can use img tag with image link. it will show the image icon Instead of the title.
			'href'  => network_admin_url( 'admin.php?page=woonet-woocommerce' ),
			'meta' => [
				'title' => __( 'Setup Wizard', 'woonet' ), //This title will show on hover
			]
		) );
	}
	...
	public function wc_multistore_migrate_settings_multisite_5_3_0(){
		if( ! $this->db_version || version_compare( $this->db_version, '5.2.9', '>' ) ){
			return;
		}
		...
		foreach ( $sites as $key => $site ){
			...
			if( ! empty( $site['settings']['child_inherit_changes_fields_control__title'] ) ){
				$sites[$key]['settings']['child_inherit_changes_fields_control_update__title'] = $site['settings']['child_inherit_changes_fields_control__title'];
			}

			if( ! empty( $site['settings']['child_inherit_changes_fields_control__slug'] ) ){
				$sites[$key]['settings']['child_inherit_changes_fields_control_update__slug'] = $site['settings']['child_inherit_changes_fields_control__slug'];
			}

			if( ! empty( $site['settings']['child_inherit_changes_fields_control__status'] ) ){
				$sites[$key]['settings']['child_inherit_changes_fields_control_update__status'] = $site['settings']['child_inherit_changes_fields_control__status'];
			}

			if( ! empty( $site['settings']['child_inherit_changes_fields_control__description'] ) ){
				$sites[$key]['settings']['child_inherit_changes_fields_control_update__description'] = $site['settings']['child_inherit_changes_fields_control__description'];
			}

			if( ! empty( $site['settings']['child_inherit_changes_fields_control__short_description'] ) ){
				$sites[$key]['settings']['child_inherit_changes_fields_control_update__short_description'] = $site['settings']['child_inherit_changes_fields_control__short_description'];
			}

			if( ! empty( $site['settings']['child_inherit_changes_fields_control__featured'] ) ){
				$sites[$key]['settings']['child_inherit_changes_fields_control_update__featured'] = $site['settings']['child_inherit_changes_fields_control__featured'];
			}

			if( ! empty( $site['settings']['child_inherit_changes_fields_control__catalogue_visibility'] ) ){
				$sites[$key]['settings']['child_inherit_changes_fields_control_update__catalogue_visibility'] = $site['settings']['child_inherit_changes_fields_control__catalogue_visibility'];
			}

			if( ! empty( $site['settings']['child_inherit_changes_fields_control__price'] ) ){
				$sites[$key]['settings']['child_inherit_changes_fields_control_update__price'] = $site['settings']['child_inherit_changes_fields_control__price'];
			}

			if( ! empty( $site['settings']['child_inherit_changes_fields_control__sale_price'] ) ){
				$sites[$key]['settings']['child_inherit_changes_fields_control_update__sale_price'] = $site['settings']['child_inherit_changes_fields_control__sale_price'];
			}

			if( ! empty( $site['settings']['child_inherit_changes_fields_control__product_tag'] ) ){
				$sites[$key]['settings']['child_inherit_changes_fields_control_update__product_tag'] = $site['settings']['child_inherit_changes_fields_control__product_tag'];
			}

			if( ! empty( $site['settings']['child_inherit_changes_fields_control__default_variations'] ) ){
				$sites[$key]['settings']['child_inherit_changes_fields_control_update__default_variations'] = $site['settings']['child_inherit_changes_fields_control__default_variations'];
			}

			if( ! empty( $site['settings']['child_inherit_changes_fields_control__sku'] ) ){
				$sites[$key]['settings']['child_inherit_changes_fields_control_update__sku'] = $site['settings']['child_inherit_changes_fields_control__sku'];
			}

			if( ! empty( $site['settings']['child_inherit_changes_fields_control__product_gallery'] ) ){
				$sites[$key]['settings']['child_inherit_changes_fields_control_update__product_gallery'] = $site['settings']['child_inherit_changes_fields_control__product_gallery'];
			}

			if( ! empty( $site['settings']['child_inherit_changes_fields_control__reviews'] ) ){
				$sites[$key]['settings']['child_inherit_changes_fields_control_update__reviews'] = $site['settings']['child_inherit_changes_fields_control__reviews'];
			}

			if( ! empty( $site['settings']['child_inherit_changes_fields_control__purchase_note'] ) ){
				$sites[$key]['settings']['child_inherit_changes_fields_control_update__purchase_note'] = $site['settings']['child_inherit_changes_fields_control__purchase_note'];
			}

			if( ! empty( $site['settings']['child_inherit_changes_fields_control__shipping_class'] ) ){
				$sites[$key]['settings']['child_inherit_changes_fields_control_update__shipping_class'] = $site['settings']['child_inherit_changes_fields_control__shipping_class'];
			}

			if( ! empty( $site['settings']['child_inherit_changes_fields_control__upsell'] ) ){
				$sites[$key]['settings']['child_inherit_changes_fields_control_update__upsell'] = $site['settings']['child_inherit_changes_fields_control__upsell'];
			}

			if( ! empty( $site['settings']['child_inherit_changes_fields_control__cross_sells'] ) ){
				$sites[$key]['settings']['child_inherit_changes_fields_control_update__cross_sells'] = $site['settings']['child_inherit_changes_fields_control__cross_sells'];
			}

			if( ! empty( $site['settings']['child_inherit_changes_fields_control__allow_backorders'] ) ){
				$sites[$key]['settings']['child_inherit_changes_fields_control__allow_backorders'] = $site['settings']['child_inherit_changes_fields_control__allow_backorders'];
			}

			if( ! empty( $site['settings']['child_inherit_changes_fields_control__menu_order'] ) ){
				$sites[$key]['settings']['child_inherit_changes_fields_control_update__menu_order'] = $site['settings']['child_inherit_changes_fields_control__menu_order'];
			}

			if( ! empty( $site['settings']['child_inherit_changes_fields_control__product_image'] ) ){
				$sites[$key]['settings']['child_inherit_changes_fields_control_update__product_image'] = $site['settings']['child_inherit_changes_fields_control__product_image'];
			}

			if( ! empty( $site['settings']['child_inherit_changes_fields_control__attributes'] ) ){
				$sites[$key]['settings']['child_inherit_changes_fields_control_update__attributes'] = $site['settings']['child_inherit_changes_fields_control__attributes'];
			}

			if( ! empty( $site['settings']['child_inherit_changes_fields_control__attribute_name'] ) ){
				$sites[$key]['settings']['child_inherit_changes_fields_control_update__attribute_name'] = $site['settings']['child_inherit_changes_fields_control__attribute_name'];
			}

			if( ! empty( $site['settings']['child_inherit_changes_fields_control__attribute_term_name'] ) ){
				$sites[$key]['settings']['child_inherit_changes_fields_control_update__attribute_term_name'] = $site['settings']['child_inherit_changes_fields_control__attribute_term_name'];
			}

			if( ! empty( $site['settings']['child_inherit_changes_fields_control__attribute_term_slug'] ) ){
				$sites[$key]['settings']['child_inherit_changes_fields_control_update__attribute_term_slug'] = $site['settings']['child_inherit_changes_fields_control__attribute_term_slug'];
			}

			if( ! empty( $site['settings']['child_inherit_changes_fields_control__attribute_term_description'] ) ){
				$sites[$key]['settings']['child_inherit_changes_fields_control_update__attribute_term_description'] = $site['settings']['child_inherit_changes_fields_control__attribute_term_description'];
			}

			if( ! empty( $site['settings']['child_inherit_changes_fields_control__variations'] ) ){
				$sites[$key]['settings']['child_inherit_changes_fields_control_update__variations'] = $site['settings']['child_inherit_changes_fields_control__variations'];
			}

			if( ! empty( $site['settings']['child_inherit_changes_fields_control__variations_data'] ) ){
				$sites[$key]['settings']['child_inherit_changes_fields_control_update__variations_data'] = $site['settings']['child_inherit_changes_fields_control__variations_data'];
			}

			if( ! empty( $site['settings']['child_inherit_changes_fields_control__variations_sku'] ) ){
				$sites[$key]['settings']['child_inherit_changes_fields_control_update__variations_sku'] = $site['settings']['child_inherit_changes_fields_control__variations_sku'];
			}

			if( ! empty( $site['settings']['child_inherit_changes_fields_control__variations_status'] ) ){
				$sites[$key]['settings']['child_inherit_changes_fields_control_update__variations_status'] = $site['settings']['child_inherit_changes_fields_control__variations_status'];
			}

			if( ! empty( $site['settings']['child_inherit_changes_fields_control__variations_stock'] ) ){
				$sites[$key]['settings']['child_inherit_changes_fields_control_update__variations_stock'] = $site['settings']['child_inherit_changes_fields_control__variations_stock'];
			}

			if( ! empty( $site['settings']['child_inherit_changes_fields_control__variations_price'] ) ){
				$sites[$key]['settings']['child_inherit_changes_fields_control_update__variations_price'] = $site['settings']['child_inherit_changes_fields_control__variations_price'];
			}

			if( ! empty( $site['settings']['child_inherit_changes_fields_control__variations_sale_price'] ) ){
				$sites[$key]['settings']['child_inherit_changes_fields_control_update__variations_sale_price'] = $site['settings']['child_inherit_changes_fields_control__variations_sale_price'];
			}

			if( ! empty( $site['settings']['child_inherit_changes_fields_control__product_cat'] ) ){
				$sites[$key]['settings']['child_inherit_changes_fields_control_update__product_cat'] = $site['settings']['child_inherit_changes_fields_control__product_cat'];
			}

			if( ! empty( $site['settings']['child_inherit_changes_fields_control__category_slug'] ) ){
				$sites[$key]['settings']['child_inherit_changes_fields_control_update__category_slug'] = $site['settings']['child_inherit_changes_fields_control__category_slug'];
			}

			if( ! empty( $site['settings']['child_inherit_changes_fields_control__category_name'] ) ){
				$sites[$key]['settings']['child_inherit_changes_fields_control_update__category_name'] = $site['settings']['child_inherit_changes_fields_control__category_name'];
			}

			if( ! empty( $site['settings']['child_inherit_changes_fields_control__category_description'] ) ){
				$sites[$key]['settings']['child_inherit_changes_fields_control_update__category_description'] = $site['settings']['child_inherit_changes_fields_control__category_description'];
			}

			if( ! empty( $site['settings']['child_inherit_changes_fields_control__category_image'] ) ){
				$sites[$key]['settings']['child_inherit_changes_fields_control_update__category_image'] = $site['settings']['child_inherit_changes_fields_control__category_image'];
			}

			if( ! empty( $site['settings']['child_inherit_changes_fields_control__category_meta'] ) ){
				$sites[$key]['settings']['child_inherit_changes_fields_control_update__category_meta'] = $site['settings']['child_inherit_changes_fields_control__category_meta'];
			}
		}

		update_site_option('wc_multistore_sites', $sites);
		$settings = get_site_option('wc_multistore_settings');
		$settings['db_version'] = '5.3.0';
		update_site_option('wc_multistore_settings', $settings);
	}

	public function wc_multistore_migrate_settings_single_master_5_3_0(){
		if( ! $this->db_version || version_compare( $this->db_version, '5.2.9', '>' ) ){
			return;
		}

		if( get_site_option('wc_multistore_network_type') != 'master' ){
			return;
		}
	...
	public function wc_multistore_migrate_settings_single_child_5_3_0(){
		if( ! $this->db_version || version_compare( $this->db_version, '5.2.9', '>' ) ){
			return;
		}

		if( get_site_option('wc_multistore_network_type') != 'child' ){
			return;
		}

		$site = get_site_option('wc_multistore_site');
		...
		if( ! empty( $site['settings']['child_inherit_changes_fields_control__title'] ) ){
			$site['settings']['child_inherit_changes_fields_control_update__title'] = $site['settings']['child_inherit_changes_fields_control__title'];
		}

		if( ! empty( $site['settings']['child_inherit_changes_fields_control__slug'] ) ){
			$site['settings']['child_inherit_changes_fields_control_update__slug'] = $site['settings']['child_inherit_changes_fields_control__slug'];
		}

		if( ! empty( $site['settings']['child_inherit_changes_fields_control__status'] ) ){
			$site['settings']['child_inherit_changes_fields_control_update__status'] = $site['settings']['child_inherit_changes_fields_control__status'];
		}

		if( ! empty( $site['settings']['child_inherit_changes_fields_control__description'] ) ){
			$site['settings']['child_inherit_changes_fields_control_update__description'] = $site['settings']['child_inherit_changes_fields_control__description'];
		}

		if( ! empty( $site['settings']['child_inherit_changes_fields_control__short_description'] ) ){
			$site['settings']['child_inherit_changes_fields_control_update__short_description'] = $site['settings']['child_inherit_changes_fields_control__short_description'];
		}

		if( ! empty( $site['settings']['child_inherit_changes_fields_control__featured'] ) ){
			$site['settings']['child_inherit_changes_fields_control_update__featured'] = $site['settings']['child_inherit_changes_fields_control__featured'];
		}

		if( ! empty( $site['settings']['child_inherit_changes_fields_control__catalogue_visibility'] ) ){
			$site['settings']['child_inherit_changes_fields_control_update__catalogue_visibility'] = $site['settings']['child_inherit_changes_fields_control__catalogue_visibility'];
		}

		if( ! empty( $site['settings']['child_inherit_changes_fields_control__price'] ) ){
			$site['settings']['child_inherit_changes_fields_control_update__price'] = $site['settings']['child_inherit_changes_fields_control__price'];
		}

		if( ! empty( $site['settings']['child_inherit_changes_fields_control__sale_price'] ) ){
			$site['settings']['child_inherit_changes_fields_control_update__sale_price'] = $site['settings']['child_inherit_changes_fields_control__sale_price'];
		}

		if( ! empty( $site['settings']['child_inherit_changes_fields_control__product_tag'] ) ){
			$site['settings']['child_inherit_changes_fields_control_update__product_tag'] = $site['settings']['child_inherit_changes_fields_control__product_tag'];
		}

		if( ! empty( $site['settings']['child_inherit_changes_fields_control__default_variations'] ) ){
			$site['settings']['child_inherit_changes_fields_control_update__default_variations'] = $site['settings']['child_inherit_changes_fields_control__default_variations'];
		}

		if( ! empty( $site['settings']['child_inherit_changes_fields_control__sku'] ) ){
			$site['settings']['child_inherit_changes_fields_control_update__sku'] = $site['settings']['child_inherit_changes_fields_control__sku'];
		}

		if( ! empty( $site['settings']['child_inherit_changes_fields_control__product_gallery'] ) ){
			$site['settings']['child_inherit_changes_fields_control_update__product_gallery'] = $site['settings']['child_inherit_changes_fields_control__product_gallery'];
		}

		if( ! empty( $site['settings']['child_inherit_changes_fields_control__reviews'] ) ){
			$site['settings']['child_inherit_changes_fields_control_update__reviews'] = $site['settings']['child_inherit_changes_fields_control__reviews'];
		}

		if( ! empty( $site['settings']['child_inherit_changes_fields_control__purchase_note'] ) ){
			$site['settings']['child_inherit_changes_fields_control_update__purchase_note'] = $site['settings']['child_inherit_changes_fields_control__purchase_note'];
		}

		if( ! empty( $site['settings']['child_inherit_changes_fields_control__shipping_class'] ) ){
			$site['settings']['child_inherit_changes_fields_control_update__shipping_class'] = $site['settings']['child_inherit_changes_fields_control__shipping_class'];
		}

		if( ! empty( $site['settings']['child_inherit_changes_fields_control__upsell'] ) ){
			$site['settings']['child_inherit_changes_fields_control_update__upsell'] = $site['settings']['child_inherit_changes_fields_control__upsell'];
		}

		if( ! empty( $site['settings']['child_inherit_changes_fields_control__cross_sells'] ) ){
			$site['settings']['child_inherit_changes_fields_control_update__cross_sells'] = $site['settings']['child_inherit_changes_fields_control__cross_sells'];
		}

		if( ! empty( $site['settings']['child_inherit_changes_fields_control__allow_backorders'] ) ){
			$site['settings']['child_inherit_changes_fields_control__allow_backorders'] = $site['settings']['child_inherit_changes_fields_control__allow_backorders'];
		}

		if( ! empty( $site['settings']['child_inherit_changes_fields_control__menu_order'] ) ){
			$site['settings']['child_inherit_changes_fields_control_update__menu_order'] = $site['settings']['child_inherit_changes_fields_control__menu_order'];
		}

		if( ! empty( $site['settings']['child_inherit_changes_fields_control__product_image'] ) ){
			$site['settings']['child_inherit_changes_fields_control_update__product_image'] = $site['settings']['child_inherit_changes_fields_control__product_image'];
		}

		if( ! empty( $site['settings']['child_inherit_changes_fields_control__attributes'] ) ){
			$site['settings']['child_inherit_changes_fields_control_update__attributes'] = $site['settings']['child_inherit_changes_fields_control__attributes'];
		}

		if( ! empty( $site['settings']['child_inherit_changes_fields_control__attribute_name'] ) ){
			$site['settings']['child_inherit_changes_fields_control_update__attribute_name'] = $site['settings']['child_inherit_changes_fields_control__attribute_name'];
		}

		if( ! empty( $site['settings']['child_inherit_changes_fields_control__attribute_term_name'] ) ){
			$site['settings']['child_inherit_changes_fields_control_update__attribute_term_name'] = $site['settings']['child_inherit_changes_fields_control__attribute_term_name'];
		}

		if( ! empty( $site['settings']['child_inherit_changes_fields_control__attribute_term_slug'] ) ){
			$site['settings']['child_inherit_changes_fields_control_update__attribute_term_slug'] = $site['settings']['child_inherit_changes_fields_control__attribute_term_slug'];
		}

		if( ! empty( $site['settings']['child_inherit_changes_fields_control__attribute_term_description'] ) ){
			$site['settings']['child_inherit_changes_fields_control_update__attribute_term_description'] = $site['settings']['child_inherit_changes_fields_control__attribute_term_description'];
		}

		if( ! empty( $site['settings']['child_inherit_changes_fields_control__variations'] ) ){
			$site['settings']['child_inherit_changes_fields_control_update__variations'] = $site['settings']['child_inherit_changes_fields_control__variations'];
		}

		if( ! empty( $site['settings']['child_inherit_changes_fields_control__variations_data'] ) ){
			$site['settings']['child_inherit_changes_fields_control_update__variations_data'] = $site['settings']['child_inherit_changes_fields_control__variations_data'];
		}

		if( ! empty( $site['settings']['child_inherit_changes_fields_control__variations_sku'] ) ){
			$site['settings']['child_inherit_changes_fields_control_update__variations_sku'] = $site['settings']['child_inherit_changes_fields_control__variations_sku'];
		}

		if( ! empty( $site['settings']['child_inherit_changes_fields_control__variations_status'] ) ){
			$site['settings']['child_inherit_changes_fields_control_update__variations_status'] = $site['settings']['child_inherit_changes_fields_control__variations_status'];
		}

		if( ! empty( $site['settings']['child_inherit_changes_fields_control__variations_stock'] ) ){
			$site['settings']['child_inherit_changes_fields_control_update__variations_stock'] = $site['settings']['child_inherit_changes_fields_control__variations_stock'];
		}

		if( ! empty( $site['settings']['child_inherit_changes_fields_control__variations_price'] ) ){
			$site['settings']['child_inherit_changes_fields_control_update__variations_price'] = $site['settings']['child_inherit_changes_fields_control__variations_price'];
		}

		if( ! empty( $site['settings']['child_inherit_changes_fields_control__variations_sale_price'] ) ){
			$site['settings']['child_inherit_changes_fields_control_update__variations_sale_price'] = $site['settings']['child_inherit_changes_fields_control__variations_sale_price'];
		}

		if( ! empty( $site['settings']['child_inherit_changes_fields_control__product_cat'] ) ){
			$site['settings']['child_inherit_changes_fields_control_update__product_cat'] = $site['settings']['child_inherit_changes_fields_control__product_cat'];
		}

		if( ! empty( $site['settings']['child_inherit_changes_fields_control__category_slug'] ) ){
			$site['settings']['child_inherit_changes_fields_control_update__category_slug'] = $site['settings']['child_inherit_changes_fields_control__category_slug'];
		}

		if( ! empty( $site['settings']['child_inherit_changes_fields_control__category_name'] ) ){
			$site['settings']['child_inherit_changes_fields_control_update__category_name'] = $site['settings']['child_inherit_changes_fields_control__category_name'];
		}

		if( ! empty( $site['settings']['child_inherit_changes_fields_control__category_description'] ) ){
			$site['settings']['child_inherit_changes_fields_control_update__category_description'] = $site['settings']['child_inherit_changes_fields_control__category_description'];
		}

		if( ! empty( $site['settings']['child_inherit_changes_fields_control__category_image'] ) ){
			$site['settings']['child_inherit_changes_fields_control_update__category_image'] = $site['settings']['child_inherit_changes_fields_control__category_image'];
		}

		if( ! empty( $site['settings']['child_inherit_changes_fields_control__category_meta'] ) ){
			$site['settings']['child_inherit_changes_fields_control_update__category_meta'] = $site['settings']['child_inherit_changes_fields_control__category_meta'];
		}

		update_site_option('wc_multistore_site', $site);
		$settings = get_site_option('wc_multistore_settings');
		$settings['db_version'] = '5.3.0';
		update_site_option('wc_multistore_settings', $settings);
	}
