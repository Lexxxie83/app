...
class WC_Multistore_Meta_Box_Product_Data {
	...
	public function add_meta_boxes( $wc_screen_id, $wc_product ) {
		if( WOO_MULTISTORE()->site->get_type() == 'master' ){
			// Add meta box for master product
			add_meta_box(
				'woo_multistore_child_products',
				__( 'Woo Multistore Child Products', 'woonet' ),
				array( $this, 'main_product_box' ),
				'product',
				'side',
				'core'
			);
		...
		if( WOO_MULTISTORE()->site->get_type() == 'child' ){
			// Add meta box for child product
			add_meta_box(
				'woo_multistore_main_product',
				__( 'Woo Multistore Parent Product', 'woonet' ),
				array( $this, 'child_product_box' ),
				'product',
				'side',
				'core'
			);
	...
	public function child_product_box(){
		require_once WOO_MSTORE_SINGLE_INCLUDES_PATH.'admin/meta-boxes/views/html-product-meta-box-child.php';
	}

	public function main_product_box(){
		require_once WOO_MSTORE_SINGLE_INCLUDES_PATH.'admin/meta-boxes/views/html-product-meta-box-master.php';
	}
