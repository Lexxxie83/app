...
class WC_Multistore_Settings{
	...
	private $defaults = array(
        ...
        'clone-products-during-import-order'									=> 'no',
	...
	public function admin_print_scripts() {
		...
		wp_register_script('wc-multistore-settings-js',WOO_MSTORE_URL . '/assets/js/wc-multistore-settings.js', array('jquery', 'jquery-ui-tabs', 'jquery-ui-sortable', 'jquery-tiptip' ),WOO_MSTORE_VERSION,true );
		...
		wp_localize_script(	'wc-multistore-settings-js','wc_multistore_ajax',array('ajax_url' => $ajax_url ) );
		wp_enqueue_script('wc-multistore-settings-js' );
	}
	...
	/**
	 * Network Admin Submenu and Master Store Admin Submenu
	 */
	...
	public function add_bar_menu_page( $admin_bar ) {
		$url = network_admin_url( 'admin.php?page=wc-multistore-settings' );
		$title = __( 'Settings', 'woonet' );
		if( ! is_multisite() && WOO_MULTISTORE()->site->get_type() != 'master' ){
			$master_data = get_site_option('wc_multistore_master_connect');
			if( ! empty( $master_data ) ){
				$url = $master_data['master_url'] . '/wp-admin/admin.php?page=wc-multistore-settings';
				$title = __( 'Settings (Main site)', 'woonet' );
			}
		}

		$admin_bar->add_menu( array(
			'id'    => 'woonet-network-settings',
