...
class WC_Multistore_Reset {
	...
	/**
	 * Network Submenu and Admin Submenu
	 */
	...
	public function add_bar_menu_page( $admin_bar ) {
		$admin_bar->add_menu( array(
			'id'    => 'woonet-network-reset',
			'parent' => 'woonet-woocommerce',
			'group'  => null,
			'title' => __( 'Reset', 'woonet' ),
			'href'  => network_admin_url( 'admin.php?page=woonet-multistore-reset' ),
			'meta' => [
				'title' => __( 'Reset', 'woonet' ),
			]
		) );
	...
	private function form_submit_settings_child() {
		...
		$wpdb->query( "DROP TABLE IF EXISTS {$wpdb->prefix}woo_multistore_attributes_relationships" );
