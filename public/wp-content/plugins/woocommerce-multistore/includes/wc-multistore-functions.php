...
if ( ! function_exists( 'wc_multistore_is_saving_order' ) ) {
	function wc_multistore_is_saving_order() {
		...
		$is_saving_order_order_items    = ( isset( $_REQUEST['action'] ) && $_REQUEST['action'] == 'woocommerce_save_order_items' );
		$is_saving_order                = doing_action('woocommerce_update_order');
		$saved_order                    = did_action('woocommerce_update_order');
		$is_multistore_saving_order     = ( isset( $_REQUEST['action'] ) && $_REQUEST['action'] == 'wc_multistore_update_master_order' );
		$is_shop_order                  = ( isset( $_REQUEST['post_type'] ) && $_REQUEST['post_type'] == 'shop_order' && isset( $_REQUEST['action'] ) );
		$is_hpos_shop_order             = ( isset( $_REQUEST['page'] ) && $_REQUEST['page'] == 'wc-orders' && isset( $_REQUEST['action'] ) );
		$is_deleting_order_item         = ( isset( $_REQUEST['action'] ) && $_REQUEST['action'] == 'woocommerce_remove_order_item' );

		if( $is_saving_order || $is_shop_order || $is_doing_refund || $is_deleting_order_item || $is_hpos_shop_order || $is_multistore_saving_order || $is_saving_order_order_items || $saved_order ){
			return true;
		}
...
if ( ! function_exists( 'wc_multistore_table_exists' ) ) {
	function wc_multistore_table_exists($tale_name) {
		global $wpdb;
...
if ( ! function_exists('wc_multistore_has_gtin_version') ){
	function wc_multistore_has_gtin_version() {
		$wc_version = WC()->version;
		return version_compare( $wc_version, '9.2.0', '>=' );
	}
}
