...
class WC_Multistore_Product_Variation_Child extends WC_Multistore_Abstract_Product_Child {
	...
	public function create() {
		...
		if ( $this->site_settings['child_inherit_changes_fields_control__variations_data'] == 'yes' ) {
			...
			if ( apply_filters( 'wc_multistore_sync_variation_description', true )  ) {
		...
		if ( wc_multistore_has_gtin_version() && $this->site_settings['child_inherit_changes_fields_control__variations_gtin'] == 'yes'){
	...
	public function update() {
		...
		if ( $this->site_settings['override__synchronize-stock'] != 'yes' && $this->site_settings['child_inherit_changes_fields_control_update__variations_stock'] == 'yes' ) {
			$this->wc_product->set_manage_stock( $this->data['manage_stock'] );
			if( isset( $this->data['stock_quantity'] ) ){
				$this->wc_product->set_stock_quantity( $this->data['stock_quantity'] );
			}else{
		...
		if ( $this->site_settings['child_inherit_changes_fields_control_update__variations_data'] == 'yes' ) {
			$this->wc_product->set_name( $this->data['post_title'] );
		...
		if ( $this->site_settings['child_inherit_changes_fields_control_update__variations_status'] == 'yes' ) {
			$this->wc_product->set_status( $this->data['post_status'] );
		}

		if ( $this->site_settings['child_inherit_changes_fields_control_update__variations_sku'] == 'yes' ) {
			if( apply_filters( 'wc_multistore_set_sku', true, $this->data, $this->wc_product ) ){
				if( empty($this->data['sku']) && WOO_MULTISTORE()->settings['sync-by-sku'] == 'yes' ){
					$this->duplicate_sku = true;
				}
		...
		if ( wc_multistore_has_gtin_version() && $this->site_settings['child_inherit_changes_fields_control_update__variations_gtin'] == 'yes'){
			if( ! empty( $this->data['gtin'] ) && ! wc_product_has_global_unique_id( $this->wc_product->get_id() , $this->data['gtin'] ) ){
				$this->duplicate_gtin = true;
			}else{
				$this->wc_product->set_global_unique_id( $this->data['gtin'] );
		...
		if ( $this->site_settings['child_inherit_changes_fields_control_update__variations_price'] == 'yes' && apply_filters( 'WOO_MSTORE_SYNC/sync_child/sync_variation_price', true ) === true ) {
			$this->wc_product->set_price( $this->data['price'] );
			$this->wc_product->set_regular_price( $this->data['regular_price'] );
		}

		if ( $this->site_settings['child_inherit_changes_fields_control_update__variations_sale_price'] == 'yes' && apply_filters( 'WOO_MSTORE_SYNC/sync_child/sync_variation_sale_price', true ) === true ) {
			$this->wc_product->set_sale_price( $this->data['sale_price'] );
