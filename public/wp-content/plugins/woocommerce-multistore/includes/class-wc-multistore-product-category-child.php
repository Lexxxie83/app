...
class WC_Multistore_Product_Category_Child extends WC_Multistore_Abstract_Term_Child {
	...
	public function update(){
		...
		if( $this->site_settings['child_inherit_changes_fields_control_update__category_slug'] == 'yes' ){
			$args['slug'] = $this->data['slug'];
		}

		if( $this->site_settings['child_inherit_changes_fields_control_update__category_name'] == 'yes' ){
			$args['name'] = $this->data['name'];
		}

		if( $this->site_settings['child_inherit_changes_fields_control_update__category_description'] == 'yes' ){
			$args['description'] = wp_kses_post( $this->data['description'] );
		}

		if( $this->site_settings['child_inherit_changes_fields_control_update__category_image'] == 'yes' ){
			update_term_meta( $this->term->term_id, 'thumbnail_id', $this->get_image_id() );
		}

		if( $this->site_settings['child_inherit_changes_fields_control_update__category_meta'] == 'yes' ){
			update_term_meta( $this->term->term_id, 'order', $this->data['order'] );
		}
